...
- Physics: the same seed gives the same throw; 1,000+ simulated dice show the rolled value; dice settle flat.
- Roll parsing: d100 splits into tens and units, unsupported dice are skipped, the dice cap applies, and hidden results are hidden.
...
- [ ] 20 dice in one roll (`/r 20d6`) on the Low quality setting: no errors, smooth playback.
